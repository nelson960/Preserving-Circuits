"""Recurrent trace-plasticity continual-learning experiment.

This is a research diagnostic for a changing plasticity mechanism with memory,
not a fixed role controller. The model has two trainable weight systems:

    theta: low-rank action adapters that alter the frozen tiny transformer
    psi:   a recurrent trace-plasticity network that decides how theta should change

The plasticity network is not trained from explicit preserve/drop/guard labels.
It sees evidence and a learned recurrent state for each trace. The recurrent
state is the experimental replacement for a frozen-base familiarity check: old
known traces, novel repeated traces, obsolete conflicts, and uncertain guarded
traces should become separable through state dynamics. It emits an independent
write gate, a competing softmax over protect/guard/drop/decay/compress state
actions, and a separate commit gate. Those gates select action adapters and
weight consequence losses. Trace state is updated while the stream is learned.
"""
    ...
    *,
    ...
    for index, person in enumerate(people):
        ...
        for name, value in trace_state.items():
            if value.ndim == 1:
                row[name] = float(value[index].detach().cpu())
            elif value.ndim == 2:
                row[f"{name}_norm"] = float(value[index].detach().norm().cpu())
            else:
                raise ValueError(f"Cannot serialize trace state {name!r} with shape {tuple(value.shape)}.")
        result[person] = row
    return result
...
class RecurrentTracePlasticityNet(nn.Module):
    def __init__(self, *, input_dim: int, hidden_dim: int, candidate_write_gain: float) -> None:
        super().__init__()
        positive_int("input_dim", input_dim)
        positive_int("hidden_dim", hidden_dim)
        positive_float("candidate_write_gain", candidate_write_gain)
        self.candidate_write_gain = float(candidate_write_gain)
        ...
        self.prediction_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(hidden_dim, len(TRACE_PREDICTION_NAMES)),
        )

    def forward(self, features: torch.Tensor, hidden: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if hidden.ndim != 2 or hidden.shape[0] != features.shape[0]:
            raise ValueError(
                "Trace hidden state must have shape [n, hidden_dim] with the same n as features; "
                f"features={tuple(features.shape)} hidden={tuple(hidden.shape)}."
            )
        next_hidden = self.cell(self.input_norm(features), hidden)
        predictions = torch.sigmoid(self.prediction_head(next_hidden))
        if predictions.shape != (features.shape[0], len(TRACE_PREDICTION_NAMES)):
            raise RuntimeError(f"Invalid plasticity prediction shape {tuple(predictions.shape)}.")
        gates = consequence_structured_gates(
            features=features,
            predictions=predictions,
            candidate_write_gain=self.candidate_write_gain,
        )
        return gates, predictions, next_hidden


def feature_column(features: torch.Tensor, name: str) -> torch.Tensor:
    if name not in TRACE_FEATURE_NAMES:
        raise ValueError(f"Unknown trace feature {name!r}; expected one of {TRACE_FEATURE_NAMES}.")
    return features[:, TRACE_FEATURE_NAMES.index(name)]


def consequence_structured_gates(
    *,
    features: torch.Tensor,
    predictions: torch.Tensor,
    candidate_write_gain: float,
) -> torch.Tensor:
    positive_float("candidate_write_gain", candidate_write_gain)
    if features.ndim != 2 or features.shape[1] != len(TRACE_FEATURE_NAMES):
        raise ValueError(f"Invalid feature shape {tuple(features.shape)}.")
    if predictions.ndim != 2 or predictions.shape[1] != len(TRACE_PREDICTION_NAMES):
        raise ValueError(f"Invalid prediction shape {tuple(predictions.shape)}.")
    prediction_by_name = {
        name: predictions[:, index]
        for index, name in enumerate(TRACE_PREDICTION_NAMES)
    }
    known = prediction_by_name["known"].clamp(0.0, 1.0)
    gain = prediction_by_name["gain"].clamp(0.0, 1.0)
    damage = prediction_by_name["damage"].clamp(0.0, 1.0)
    stability = prediction_by_name["stability"].clamp(0.0, 1.0)
    recurrence = feature_column(features, "recurrence").clamp(0.0, 1.0)
    conflict = feature_column(features, "conflict").clamp(0.0, 1.0)
    stream_reference = feature_column(features, "stream_reference").clamp(0.0, 1.0)
    capacity_pressure = feature_column(features, "capacity_pressure").clamp(0.0, 1.0)
    protected = feature_column(features, "protected").clamp(0.0, 1.0)
    known_memory = feature_column(features, "known_memory").clamp(0.0, 1.0)
    stability_memory = feature_column(features, "stability_memory").clamp(0.0, 1.0)
    age_norm = feature_column(features, "age_norm").clamp(0.0, 1.0)
    usefulness_memory = feature_column(features, "usefulness").clamp(0.0, 1.0)
    usefulness = (recurrence + stream_reference + usefulness_memory).clamp(0.0, 1.0)
    non_conflict = (1.0 - conflict).clamp(0.0, 1.0)
    effective_known = torch.maximum(known, known_memory).clamp(0.0, 1.0)
    effective_stability = torch.maximum(stability, stability_memory * effective_known).clamp(0.0, 1.0)

    raw_write = (gain * (1.0 - effective_known) * (1.0 - damage) * non_conflict).clamp(0.0, 1.0)
    candidate_amplifier = 1.0 + (candidate_write_gain - 1.0) * (1.0 - effective_known).clamp(0.0, 1.0)
    write_gate = (raw_write * candidate_amplifier).clamp(0.0, 1.0)
    protect_raw = (effective_known * effective_stability * usefulness * non_conflict).clamp(0.0, 1.0)
    drop_raw = (conflict * (1.0 - usefulness).clamp(0.0, 1.0)).clamp(0.0, 1.0)
    uncertainty = (1.0 - torch.maximum(torch.maximum(effective_known, gain), effective_stability)).clamp(0.0, 1.0)
    guard_raw = (damage + protected * (1.0 - effective_stability).clamp(0.0, 1.0) + uncertainty * non_conflict).clamp(
        0.0,
        1.0,
    )
    decay_raw = (
        capacity_pressure
        * age_norm
        * (1.0 - usefulness).clamp(0.0, 1.0)
        * (1.0 - write_gate).clamp(0.0, 1.0)
    ).clamp(0.0, 1.0)
    compress_raw = (
        capacity_pressure
        * effective_known
    ...
    state_raw = torch.stack([protect_raw, guard_raw, drop_raw, decay_raw, compress_raw], dim=1).clamp_min(1e-6)
    state_gates = state_raw / state_raw.sum(dim=1, keepdim=True)
    commit_gate = (
        effective_known
        * effective_stability
        * usefulness
    ...
    gates = torch.cat([write_gate.unsqueeze(1), state_gates, commit_gate.clamp(0.0, 1.0)], dim=1)
    if gates.shape != (features.shape[0], len(TRACE_GATE_NAMES)):
        raise RuntimeError(f"Invalid structured gate shape {tuple(gates.shape)}.")
    return gates
...
def make_initial_trace_state(
    *,
    ...
    return {
        ...
        "known": 0.05 + 0.90 * old_mask,
        "stability": 0.02 + 0.80 * old_mask,
        ...
        "hidden": torch.zeros((count, hidden_dim), dtype=torch.float32, device=device),
    }
...
def trace_features(
    *,
    ...
    with torch.no_grad():
        provisional_features = torch.stack(
            [
        ...
        provisional, _provisional_predictions, _provisional_hidden = plasticity(
    ...
    features = torch.cat(
        [
            ...
            trace_state["known"].unsqueeze(1),
            trace_state["stability"].unsqueeze(1),
...
def trainable_parameters(
    *,
    ...
    plasticity: RecurrentTracePlasticityNet | None,
) -> list[nn.Parameter]:
...
def train_recurrent_stage(
    *,
    ...
    seed: int,
    label: str,
) -> tuple[list[dict[str, float]], torch.Tensor, torch.Tensor, dict[str, dict[str, float]]]:
    ...
    latest_predictions: torch.Tensor | None = None
    latest_metrics: dict[str, dict[str, float]] | None = None
    latest_hidden: torch.Tensor | None = None
    for epoch in range(1, args.stage_epochs + 1):
        permutation = torch.randperm(len(stage_examples), generator=generator)
        totals = {
            "loss": 0.0,
        ...
        for start in pbar:
            ...
            gates, predictions, next_hidden = plasticity(features, trace_state["hidden"].detach())
            latest_gates = gates.detach()
            latest_predictions = predictions.detach()
            latest_hidden = next_hidden.detach()
            ...
            damage_from_loss = ((protect_losses.detach() + guard_losses.detach()) / (2.0 * args.loss_clip)).clamp(
                min=0.0,
                max=1.0,
            )
            damage_from_state = (
                trace_state["protected"].detach()
                * non_conflict
                * (1.0 - recurrence).clamp(min=0.0, max=1.0)
            ).clamp(min=0.0, max=1.0)
            known_target = torch.maximum(
                learned_stable,
                trace_state["known"].detach(),
            ).clamp(min=0.0, max=1.0)
            gain_target = (loss_norm * non_conflict * (1.0 - known_target).clamp(min=0.0, max=1.0)).clamp(
                min=0.0,
                max=1.0,
            )
            damage_target = torch.maximum(damage_from_loss, damage_from_state).clamp(min=0.0, max=1.0)
            stability_target = (
                torch.maximum(learned_stable, trace_state["known"].detach())
                * (trace_state["stability"].detach() + trace_state["protected"].detach() + trace_state["strength"].detach()).clamp(max=1.0)
                * non_conflict
            ).clamp(min=0.0, max=1.0)
            consequence_target = torch.stack(
                [
                    known_target,
                    gain_target,
                    damage_target,
            ...
            consequence_alignment = F.binary_cross_entropy(
                predictions.clamp(min=1e-6, max=1.0 - 1e-6),
                consequence_target,
            )
            prediction_by_name = {
                name: predictions[:, index]
            ...
            target_gates = consequence_structured_gates(
                features=features.detach(),
                predictions=consequence_target,
                candidate_write_gain=args.candidate_write_gain,
            )
            target_gate_by_name = {
                name: target_gates[:, index].detach()
                for index, name in enumerate(TRACE_GATE_NAMES)
            }
            write_target = target_gate_by_name["write"]
            write_alignment = F.binary_cross_entropy(
                gate_by_name["write"].clamp(min=1e-6, max=1.0 - 1e-6),
                write_target,
            )
            state_target = torch.stack(
                [
                    target_gate_by_name["protect"],
                    target_gate_by_name["guard"],
                    target_gate_by_name["drop"],
                    target_gate_by_name["decay"],
                    target_gate_by_name["compress"],
                ],
                dim=1,
            )
            ...
            commit_target = target_gate_by_name["commit"]
            ...
            loss = (
                ...
                + args.lambda_consequence_prediction * consequence_alignment
            ...
            optimizer.step()
            trace_state["hidden"] = next_hidden.detach()
            ...
            row = {
                ...
                "consequence": float(consequence_alignment.detach().cpu()),
                "maturity": float(maturity.mean().detach().cpu()),
                "novelty": float(novelty.mean().detach().cpu()),
                "known": float(prediction_by_name["known"].mean().detach().cpu()),
                "gain": float(prediction_by_name["gain"].mean().detach().cpu()),
                "damage": float(prediction_by_name["damage"].mean().detach().cpu()),
                "stability": float(prediction_by_name["stability"].mean().detach().cpu()),
        ...
        if epoch == 1 or epoch == args.stage_epochs or epoch % args.print_every == 0:
            print(
                ...
                f"known={epoch_row['known']:.3f} gain={epoch_row['gain']:.3f} "
                f"damage={epoch_row['damage']:.3f} commit={epoch_row['commit']:.3f} "
                f"decay={epoch_row['decay']:.3f}"
            )
    if latest_gates is None or latest_predictions is None or latest_metrics is None or latest_hidden is None:
        raise RuntimeError(f"{label} produced no gates.")
    return trace, latest_gates, latest_predictions, latest_metrics
...
def update_trace_state(
    *,
    ...
    after_token_accuracy = torch.tensor(
        [float(after_metrics[p]["token_accuracy"]) for p in people],
        dtype=torch.float32,
        device=device,
    )
    gain = (before_loss - after_loss).clamp(min=-args.loss_clip, max=args.loss_clip) / args.loss_clip
    after_known_evidence = ((1.0 - (after_loss / args.loss_clip).clamp(0.0, 1.0)) * after_token_accuracy).clamp(0.0, 1.0)
    useful = (
    ...
    strength_delta = (
        ...
        + args.trace_candidate_rate
        * gate_by_name["write"]
        * gain.clamp_min(0.0)
        * (stream_reference + recurrence).clamp(max=1.0)
    ...
    known = (
        args.trace_ema * trace_state["known"]
        + (1.0 - args.trace_ema) * after_known_evidence
        + args.trace_commit_rate * gate_by_name["commit"] * useful
        - args.trace_decay_rate * gate_by_name["drop"] * conflict
    ).clamp(0.0, 1.0)
    stability = (
        args.trace_ema * trace_state["stability"]
        + (1.0 - args.trace_ema) * after_known_evidence * useful * (1.0 - conflict).clamp(0.0, 1.0)
        + args.trace_commit_rate * (gate_by_name["protect"] + gate_by_name["commit"]) / 2.0
        - args.trace_decay_rate * (gate_by_name["drop"] + gate_by_name["decay"]) / 2.0
    ).clamp(0.0, 1.0)
    return {
        ...
        "hidden": trace_state["hidden"].detach(),
    }
    ...
    *,
    ...
    if method == "naive":
        ...
        for stage_number, stage_name in enumerate(["stage2", "stage3"], start=2):
            ...
            del before_features
            trace, final_gates, final_predictions, _during_metrics = train_recurrent_stage(
                args=args,
                adapter=adapter,
                plasticity=plasticity,
                people=people,
            ...
            stage_reports.append(
                {
                    ...
                    "trace_state": {
                        **serialize_trace_state(people=people, trace_state=trace_state)
                    },
                }
            )
    else:
...
def run(args: argparse.Namespace) -> None:
    ...
    print("TINY RECURRENT TRACE-PLASTICITY CL")
    print("=" * 120)
    ...
    print("\nTINY RECURRENT TRACE-PLASTICITY CL SUMMARY")
    print("=" * 120)
    print(
    ...
    if "recurrent" in results:
        print("\nRECURRENT TRACE STATE")
        print("-" * 120)
        for person, state in results["recurrent"]["trace_state"].items():
            gates = results["recurrent"]["final_gates"][person]
            predictions = results["recurrent"]["final_predictions"][person]
            print(
                f"{person:>6} pred={results['recurrent']['predicted_roles'][person]:<8} "
                ...
                f"known={predictions['known']:.3f} gain={predictions['gain']:.3f} "
                f"damage={predictions['damage']:.3f} stable={predictions['stability']:.3f} "
                f"h={state['hidden_norm']:.3f}"
            )

    output = {
        "question": "Can recurrent trace-plasticity memory learn trace hardening, decay, suppression, and protection from evidence over time?",
        "config": {
            "args": {
            ...
            "trace_prediction_names": list(TRACE_PREDICTION_NAMES),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-json", type=Path, default=Path("model/analysis/gco-tiny-recurrent-trace-plasticity-seed0.json"))
    parser.add_argument("--methods", type=str, default="naive,recurrent")
    ...
    parser.add_argument("--lambda-consequence-prediction", type=float, default=0.5)
